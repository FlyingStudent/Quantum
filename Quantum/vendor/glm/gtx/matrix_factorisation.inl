...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER void qr_decompose(mat<C, R, T, Q> const& in, mat<(C < R ? C : R), R, T, Q>& q, mat<C, (C < R ? C : R), T, Q>& r)
	{
		...
		for (length_t i = 0; i < (C < R ? C : R); i++)
		{
			...
			//j = [0,i[
			// Make that column orthogonal to all the lastious ones by substracting to it the non-orthogonal projection of all the lastious columns.
			...
			//Now, Q i-th column is orthogonal to all the lastious columns. Normalize it.
			q[i] = normalize(q[i]);
