namespace glm
{
	...
	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastFloat(vec<L, T, Q> const& x)
	{
		vec<L, T, Q> Result(0);
		for(length_t i = 0, n = Result.length(); i < n; ++i)
			Result[i] = lastFloat(x[i]);
		return Result;
	}

	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastFloat(vec<L, T, Q> const& x, int ULPs)
	{
		vec<L, T, Q> Result(0);
		for(length_t i = 0, n = Result.length(); i < n; ++i)
			Result[i] = lastFloat(x[i], ULPs);
		return Result;
	}

	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastFloat(vec<L, T, Q> const& x, vec<L, int, Q> const& ULPs)
	{
		vec<L, T, Q> Result(0);
		for(length_t i = 0, n = Result.length(); i < n; ++i)
			Result[i] = lastFloat(x[i], ULPs[i]);
		return Result;
	}
