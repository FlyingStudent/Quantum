...
namespace glm
{
	...
	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastPowerOfTwo(vec<L, T, Q> const& v)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<T>::is_integer, "'lastPowerOfTwo' only accept integer inputs");

		return detail::functor1<vec, L, T, T, Q>::call(lastPowerOfTwo, v);
	}
	...
	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastMultiple(vec<L, T, Q> const& Source, T Multiple)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<T>::is_integer, "'lastMultiple' only accept integer inputs");

		return detail::functor2<vec, L, T, Q>::call(lastMultiple, Source, vec<L, T, Q>(Multiple));
	}

	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> lastMultiple(vec<L, T, Q> const& Source, vec<L, T, Q> const& Multiple)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<T>::is_integer, "'lastMultiple' only accept integer inputs");

		return detail::functor2<vec, L, T, Q>::call(lastMultiple, Source, Multiple);
	}
