...
namespace  YAML
{
	...
	struct convert<glm::vec3>
	{
		...
		//template<>
		//struct convert<Quantum::UUID>
		//{
		//	static Node encode(const Quantum::UUID& uuid)
		//	{
		//		Node node;
		//		node.push_back((uint64_t)uuid);
		//		return node;
		//	}
		//	static bool decode(const Node& node, Quantum::UUID& uuid)
		//	{
...
namespace Quantum {
 ...
 YAML::Emitter& operator<<(YAML::Emitter& out, const glm::vec4& v)
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		out << YAML::Key << "Entity" << YAML::Value << "123456789";//TO DO: UUID
		...
		if (entity.HasComponent<CameraComponent>())
		{
			...
			out << YAML::Key << "PerspectiveFOV" << YAML::Value << (float)camera.GetPerspectiveVerticalFOV();
	...
	bool SceneSerializer::DeSerialize(const std::string& filepath)
	{
		...
		{
			QT_CORE_ERROR("Failed to load .quantum file '{0}'\n		{1}", filepath, e.what());
			return false;
		}
		...
		QT_CORE_TRACE("DeSerialized Scene '{0}'", sceneName);
