...
namespace Quantum {
	...
	namespace Utils {
		...
		static GLenum	QuantumFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			...
			QT_CORE_ASSERT(false, "Unknown format!");
			return 0;
		}
	 }
	...
	OpenGLFramebuffer::~OpenGLFramebuffer()
	{
		...
		GLValidate(glDeleteTextures(static_cast<GLsizei>(m_ColorAttachments.size()), m_ColorAttachments.data()));
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			QT_CORE_ASSERT(m_ColorAttachments.size() <= 4, "ColorAttachments Size <=4!");
	...
	{
		QT_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "attachmentIndex >= m_ColorAttachments.size() !");
